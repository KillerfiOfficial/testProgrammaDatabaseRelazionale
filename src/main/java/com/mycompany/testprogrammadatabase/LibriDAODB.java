...
public class LibriDAODB implements LibriDAO {
    ...
    @Override
    public void editLibri(Connection conn, int isbnOld, int isbnNew, String titolo, String casaeditrice, int idAutore, int numeroPagine, String lingua, String genere, double prezzo) {

        String query = "UPDATE LIBRI SET TITOLO = ?, CASAEDITRICE = ?, NUMPAGINE = ?, LINGUA = ?, GENERE = ?, PREZZO = ?, ISBN = ? WHERE ISBN = ?";
        ...
        try (PreparedStatement pst = conn.prepareStatement(query)) {
            ...
            pst.setInt(7, isbnNew);
            pst.setInt(8, isbnOld);
            
            pst.executeUpdate();
            
            updateLibreria(conn, isbnOld, isbnNew);
    ...
    void updateLibreria(Connection conn, int isbnOld, int isbnNew) throws SQLException {
        String updateQuery = "UPDATE LIBRERIA SET ISBN = ? WHERE ISBN = ?";
        
        try (PreparedStatement pst = conn.prepareStatement(updateQuery)){
            pst.setInt(1, isbnNew);
            pst.setInt(2, isbnOld);
            int rowsUpdated = pst.executeUpdate();
            logger.info("Numero di righe aggiornate in Libreria: " + rowsUpdated);
        } catch (SQLException e) {
            e.printStackTrace();
            logger.error("Errore nell'aggiornamento nella tabella Libreria: " + e.getMessage());
            throw e;
        }
    }
