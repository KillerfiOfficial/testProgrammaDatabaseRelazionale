...
public class LibroDAODB implements LibroDAO {
    ...
    public void addLibreria(Connection conn, int id, String isbn, int idAutore, int scaffale, int quantita, int venduti, int classifica, String recensione) {
        try {
            if (isbnExists(conn, isbn)) {
                logger.error("ISBN già presente nel database. Impossibile aggiungere il libro.");
                return; 
            }

            if (!autoreExists(conn, idAutore)) {
                logger.error("ID autore non presente nel database. Impossibile aggiungere il libro.");
                return; 
            }
    ...
    private boolean autoreExists(Connection conn, int idAutore) throws SQLException {
        String sql = "SELECT COUNT(*) AS count FROM AUTORI WHERE IDAUTORE = ?";
        try (PreparedStatement pst = conn.prepareStatement(sql)) {
    ...
    private boolean isbnExists(Connection conn, String isbn) throws SQLException {
        String sql = "SELECT COUNT(*) AS count FROM LIBRERIA WHERE ISBN = ?";
        try (PreparedStatement pst = conn.prepareStatement(sql)) {
            pst.setString(1, isbn);
